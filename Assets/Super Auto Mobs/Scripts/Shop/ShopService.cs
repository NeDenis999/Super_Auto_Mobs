...
namespace Super_Auto_Mobs
{
    public class ShopService : Shop
    {
        ...
        public void CreatePlatformMobs()
        {
            for (int i = 0; i < _sessionProgressService.MyCommandMobsData.Count; i++)
            {
                _mobFactoryService.CreateMobInPlatform(_assetProviderService
                    .MobPrefab(_sessionProgressService.MyCommandMobsData[i].MobEnum), _commandPetPlatforms[i], 
                    _sessionProgressService.MyCommandMobsData[i]);
            }
        }
        
        public void DestroyPlatformMobs()
        {
